    ...
    <el-dialog v-model="dialogTableVisible" title="文章内容">
      ...
      <template #footer>
        <span class="dialog-footer">
          ...
          <el-button plain @click="open" v-if="UserData.id == form.article.authorId">删除</el-button>
        </span>
      </template>
    </el-dialog>
...
const open = () => {
  ElMessageBox.confirm(
    ...
    .then(() => {
      axios.get('/article/delUserArticle/' + form.article.id).then(response => {
        console.log(form.messages)

        if (response.data.code == 200) {
          data.data =  data.data.filter(item => item.id != form.article.id)
          ElMessage({
            type: 'success',
